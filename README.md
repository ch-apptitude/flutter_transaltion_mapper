...
## Advanced: External Translation Provider

If you're using a service that provides over-the-air (OTA) translation updates (like Lokalise, Phrase, Transifex, or custom APIs), you can integrate it with this package by setting an external translation provider.

### When to Use External Translation Provider

Use this feature when:
- You need to update translations without rebuilding/redeploying your app
- You're using OTA translation services (Lokalise, Phrase, etc.)
- You want to fetch translations from a remote API or database
- You need dynamic translations that change at runtime

### How to Set Up

Set the `externalTranslationProvider` callback in your app initialization:

```dart
import 'package:flutter_translation_mapper/custom_localization.dart';
import 'package:lokalise_flutter_sdk/lokalise_flutter_sdk.dart'; // or any other service

Future<void> main() async {
  WidgetsFlutterBinding.ensureInitialized();
  
  // Initialize your OTA translation service
  await Lokalise.init(
  ...
  // Connect it to your translation package
  CustomLocalization.externalTranslationProvider = (String key) {
    // Return the translation from your service, or null to fall back to .arb files
    return Lokalise.getString(key);
  };
...
When you call `context.translate('key')`, the package will:
1. **First**: Check the external provider (if set)
2. **Second**: Fall back to bundled .arb files
3. **Last**: Return `??:key` if not found

#### Clearing External Provider
```dart
// Revert to .arb files only
CustomLocalization.externalTranslationProvider = null;
```
